...
def Generate_XML(article, title, url, group, img_path, typ="", id_="", outDir="",
    ...
    with io.open(out_file, "w", encoding="utf-8") as output_f:
        ...
        output_f.write('<field name="userRating">0</field>' + "\n")
        output_f.write('<field name="callToAction">0</field>' + "\n")
        output_f.write('<field name="isActive">true</field>' + "\n")
        output_f.write('<field name="contributor">alghiwot@gmail.com</field>' + "\n")
...
def PARSE_URDU_TEXT(s):
    ...
    for word in str_array:
        ...
        if unicode_vale >= 0 and unicode_vale <= 999999 or unicode_vale >= 48 and unicode_vale <= 57:
