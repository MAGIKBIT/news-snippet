...
def CheckExistanceImSolr(url, retry_limit):
    count = 0
    numFound = None
    query = "http://" + solr_info["ip"] + ":" + solr_info["port"] + "/solr/" + solr_info["core"] + "/" + "select?df=sourceUrl&indent=on&q=" + "\""+ url + "\"&wt=json"
    print("SolrIndex ---> " + query)
...
if __name__ == '__main__':
    ...
    if general_conf is None:
        raise RuntimeError("Missing 'general' section in configuration file. Please check ../config/configuration.ini.")
    ...
    for src in urls:
        ...
        while len(article_urls) > 0 and not _quit :
            if not obj.isDuplicateUrl(article_urls[0]):
                ...
                if numFound == None : #numFound != None and numFound == 0:
